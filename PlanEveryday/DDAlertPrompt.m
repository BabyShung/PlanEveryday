...
- (id)initWithTitle:(NSString *)title delegate:(id /*<UIAlertViewDelegate>*/)delegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitles andCategoryName:(NSString *) name andindexInArray:(NSInteger) index{
    ...
    self.indexInMutableArray = index;
...
- (UITextField *)plainTextField {
	...
	if (!plainTextField_) {
		plainTextField_ = [[UITextField alloc] initWithFrame:CGRectMake(10.0f, 0.0f, 255.0f, 58.0f)];
		plainTextField_.contentVerticalAlignment = UIControlContentHorizontalAlignmentCenter;
        plainTextField_.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
		plainTextField_.clearButtonMode = UITextFieldViewModeWhileEditing;
		plainTextField_.placeholder = @"New Category";
        
        //modify
        if(![self.CategoryName isEqualToString:nil])
        {
            plainTextField_.text = self.CategoryName;
        }
