...
-(void)reloadCollapseClick {
    ...
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
        ...
        if ([(id)CollapseClickDelegate respondsToSelector:@selector(colorForTitleArrowAtIndex:)]) {
            ...
            [cell.TitleAdd drawWithColor:[CollapseClickDelegate colorForTitleArrowAtIndex:xx]];
        ...
        //hao modified
        // Add target to Button add
        [cell.AddBtn addTarget:self action:@selector(didSelectCollapseClickAddButtonByHao:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)didSelectCollapseClickAddButtonByHao:(UIButton *)addButton {

    
    // Call delegate method
    if ([(id)CollapseClickDelegate respondsToSelector:@selector(didClickCollapseClickCellAtIndex:isNowOpen:)]) {
        [CollapseClickDelegate didClickCollapseClickCellAddBtnAtIndexByHao:addButton.tag];
    }
}
